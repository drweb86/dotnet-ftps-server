...
/// Idle and sleep are taken as separate locks so the service still inhibits idle suspend
/// even when block-sleep is denied by polkit (e.g. no active logind session).
/// </summary>
...
internal sealed partial class LinuxOsSleepPreventionService : IOsSleepPreventionService
{
    ...
    private const string Who = "FtpsServer";
    private const string Why = "FTPS server is running";
    ...
    private void TakeInhibitLocks()
    {
        ...
        foreach (var what in InhibitWhat)
        {
            ...
            {
                // sleep:block is often denied without an active logind session (headless/cron);
                // idle:block is usually allowed. Keep whatever succeeded.
            }
        }
    ...
    private void ReleaseInhibitLocks()
    {
        foreach (var handle in _inhibitFds)
        {
            ...
            {
                // Closing the logind fd is best-effort; the server must still be able to stop.
            }
        }
        _inhibitFds.Clear();
