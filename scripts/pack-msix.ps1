# Pack unsigned Store MSIX from published FtpsServerWindows output (x64 and arm64).
...
$packageDir = Join-Path $RepoRoot "sources\FtpsServerWindows.Package"
...
foreach ($arch in $arches) {
    ...
    $exe = Join-Path $publishDir "FtpsServerWindows.exe"
    ...
    $msixName = "ftpsserver_${Version}_windows_$($arch.Folder).msix"
